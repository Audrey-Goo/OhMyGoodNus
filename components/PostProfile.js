...
import { BookmarkIcon,ChatIcon,DotsHorizontalIcon,EmojiHappyIcon,
    ...
    function PostProfile({ id,username, userImg, img, caption }) {
      ...
      useEffect(() => onSnapshot(query(collection(db,"profilePosts",id,"comments"), orderBy("timestamp",'desc')), (snapshot) => setComments(snapshot.docs),[db]))
    
      useEffect(() => onSnapshot(collection(db, "profilePosts", id, "likes"), (snapshot) => setLikes(snapshot.docs)),[db, id])
    
      useEffect(() => setHasLiked(likes.findIndex((like) => like.id === session?.user?.uid) !== -1),[likes])
    
      const likePost = async () => {
        if (hasLiked) {
          await deleteDoc(doc(db, 'profilePosts', id, 'likes', session.user.uid))
        } else {
          await setDoc(doc(db, 'profilePosts', id, 'likes', session.user.uid), {
      ...
      const sendComment = async (e) => {
        ...
        await addDoc(collection(db, 'profilePosts', id, 'comments'), {
