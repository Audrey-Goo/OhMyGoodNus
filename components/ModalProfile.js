...
function ModalProfile() {
    ...
    const uploadPost = async () => {
        ...
        const imageRef = ref(storage, `profilePosts/${docRef.id}/image`)

        await uploadString(imageRef, selectedFile, "data_url").then(async snapshot => {
            const downloadURL = await getDownloadURL(imageRef);
            await updateDoc(doc(db, 'profilePosts', docRef.id), {
                image: downloadURL
            })
        })
            ...
            sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
              <div>
                ...
                <div className='mt-5 sm:mt-6'>
                  <button type="button" disabled={!selectedFile} className='inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2
                  ...
                  sm:text-sm disabled:bg-gray-300 disabled:cursor-not-allowed hover:disabled:bg-gray-300' onClick={uploadPost}>
